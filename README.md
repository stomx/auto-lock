...
3. **단일 임계값** + 무신호 허용시간 + 카운트다운 시간으로 잠금 결정
   - RSSI ≥ 임계값 → 근접 (정상 사용 중)
   - RSSI < 임계값 → 설정된 카운트다운 즉시 시작
   - 신호가 안 잡히면 설정된 무신호 허용시간을 기다린 뒤 카운트다운 시작
   - 카운트다운이 끝까지 진행되면 잠금
...
- 화면 오버레이는 설정된 카운트다운 전체를 표시합니다(기본 5 → 4 → 3 → 2 → 1).
...
3. **거리 임계값** 슬라이더 — −40 ~ −100 dBm, **5 dBm 단위** 조정
   - 기본값은 **−80 dBm** (권장 범위 −75 ~ −85 dBm의 중앙). 거리에 따라 직접 측정해보고 조정
   - 즉시잠금 임계값은 자동으로 `임계값 - 10 dBm` (예: 임계값 -80 → 즉시잠금 -90 dBm)
4. **무신호 허용 시간** — 5~60초, 5초 단위. 기본값 **10초**
5. **카운트다운 시간** — 1~30초, 1초 단위. 기본값 **5초**
6. **로그인 시 자동 시작** / **근접 시 화면 깨우기** 토글

기본 설정에서는 마지막 BLE 광고 후 10초 동안 신호 회복을 기다리고, 이어서
5초 카운트다운을 표시한 뒤 약 15초 시점에 잠급니다.
...
상태 화면과 SQLite/Unified Logging 이벤트를 이용한 장애 분석 방법은
[docs/LOGGING.md](docs/LOGGING.md)를 참고하세요.
