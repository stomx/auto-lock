...
- 누르면 해당 릴리스의 `arm64` DMG를 내려받아 **SHA256SUMS.txt와 대조 검증**한 뒤
  디스크 이미지를 자동으로 엽니다. 검증에 실패하면(체크섬 불일치·누락·다운로드
  실패) 업데이트를 중단합니다.
- 열린 창에서 AutoLock을 **Applications 폴더로 드래그**하고 앱을 다시 실행하면 끝입니다.

> 설치 자체를 앱이 강제로 교체하지는 않습니다. ad-hoc 서명 빌드는 자동 교체 시
> 매번 Gatekeeper 경고가 떠서, "다운로드·검증·열기"까지만 자동화하고 마지막
> 드래그 설치는 사용자가 합니다. 정식 Developer ID 서명 + notarization을 도입하면
> Sparkle 같은 완전 자동 업데이트로 확장할 수 있습니다.
