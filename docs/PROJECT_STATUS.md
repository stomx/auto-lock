...
> ⚠️ **이 문서는 v0.2.0(2026-05-28) 시점 스냅샷입니다.** 이후 v0.3.0에서
> 3계층 아키텍처 분리 + 테스트 인프라(swift-testing)가 도입됐고, 여기 적힌
> "코드 구조"(평면 `Sources/AutoLock/`)와 "CRITICAL 미해결" 목록 상당수는
> 더 이상 유효하지 않습니다. **최신 현황·구조는 [00-HANDOFF.md](00-HANDOFF.md)와
> 루트 [README.md](../README.md) "구조" 섹션, [CHANGELOG.md](../CHANGELOG.md)를
> 참조하세요.** 이 문서는 당시 코드 리뷰 결과의 기록으로 보존합니다.
