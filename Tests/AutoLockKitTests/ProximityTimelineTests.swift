...
@Suite struct ProximityTimelineTests {
    ...
    private func makeSettings(grace: Int, magnitude: Int) -> Settings {
    ...
    private func replay(
        ...
        let settings = makeSettings(grace: grace, magnitude: magnitude)
    ...
    @Test func awayByStaleThenLocks() {
        let frames = replay(grace: 15, magnitude: 70, through: 40) { tick in
    ...
    @Test func returnsToNearBeforeLock() {
        let frames = replay(grace: 15, magnitude: 70, through: 30) { tick in
    ...
    // 시나리오1과 달리 lastSeen이 항상 fresh라 stale이 아닌 rssi 분기로 잠긴다.
    @Test func weakeningSignalCrashesToLock() {
        let frames = replay(grace: 15, magnitude: 70, through: 15) { tick in
    ...
    // awaySince=t21 → deadline=t36. remaining<=5 인 t31~t35에서 show.
    @Test func staleCountdownShowsOverlayInFinalWindow() {
        let overlay = SpyOverlay()
        let frames = replay(grace: 15, magnitude: 70, through: 40, overlay: overlay) { tick in
            tick <= 5 ? (rssi: -50, lastSeenTick: tick) : nil
        }
