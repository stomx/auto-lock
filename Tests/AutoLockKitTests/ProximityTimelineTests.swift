...
@Suite struct ProximityTimelineTests {
    ...
    /// grace는 더 이상 사용자 조정 항목이 아니라 10초 고정. 타임라인 테스트도
    /// 이 고정값으로 동작/검증한다.
    ...
    private func makeSettings(magnitude: Int) -> Settings {
        ...
        // grace는 Settings.gracePeriodSeconds가 항상 10초 고정으로 반환.
    ...
    @Test func awayByStaleThenLocks() {
        // grace=10 → absence=20, prune=22. lastSeen=5.
        ...
        #expect(frames.prefix(16).allSatisfy { $0.lockCount == 0 })

        // age > grace(10) 진입 구간(lastSeen=5 → tick16에서 age11)부터 카운트다운.
        #expect(isCountdown(frames[16].status))
        #expect(frames[16].state == .borderline)
        ...
        // 실측 경계: age>absence(20) 첫 틱 = t26(age21). prune은 t28이라 아직 존재.
        #expect(lockFrame.tick == 26)
    }

    // MARK: - 2. 이탈 중 복귀 → 잠금 안 됨
    //
    // grace=10. tick0~5 near. tick6~23 광고 중단(카운트다운 진입까지 감).
    // tick24부터 강신호 광고 재개 → awaySince 리셋 → near 회복, 잠금 0.
    // (lastSeen=5, age>absence(20)은 t26부터라 복귀 t24가 더 빠르다 → 잠금 0 유지.)
    ...
    //
    // grace=10, threshold -70(defAway -80). 디바이스는 계속 광고(lastSeen 매
    ...
    // grace=10: awaySince=t16 → deadline=t26. remaining<=5 인 t21~t25에서 show.
    @Test func staleCountdownShowsOverlayInFinalWindow() {
        let overlay = SpyOverlay()
        // through:26 — t26 instant-lock 직후까지만 본다. 그 이후엔 디바이스가
        // pruned되며 deviceUnseen 카운트다운이 새로 시작해 오버레이를 다시 열어
        // lastDeadline을 덮으므로(이 테스트의 관심사 아님) 잠금 시점에서 끊는다.
        let frames = replay(magnitude: 70, through: 26, overlay: overlay) { tick in
            tick <= 5 ? (rssi: -50, lastSeenTick: tick) : nil
        }

        // final window 진입 전(카운트다운 시작 t16 ~ t20)에는 show가 한 번도 안 불린다.
        #expect(frames.first { $0.showCount > 0 }!.tick == 21)
        #expect(frames.prefix(21).allSatisfy { $0.showCount == 0 })

        // final window(t21~t25)에서 show가 실제로 호출되고 deadline이 잡힌다.
        #expect(overlay.showCount >= 1)
        #expect(overlay.lastDeadline != nil)

        // deadline은 awaySince(t16) + grace(10) = t0 + 26초.
        let expectedDeadline = t0.addingTimeInterval(26)
